...
Unit, native parser and isolated Linux SSH integration tests cover the implemented boundaries. Hosted Windows Server 2022 installation, SSH/SFTP, ACLs, retries and cleanup are covered in both privilege modes. Physical Windows 10/11 enrollment, reboot, sleep and battery recovery require a target acceptance run; transport tests do not substitute for those observations. The current evidence is recorded in test/test-report.md.
